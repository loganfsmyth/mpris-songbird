...
package32: build
	sed '7s/MPRIS/MPRIS (32-bit)/' src/install.rdf | sed '5s/UUID/$(UUID32)/' | sed '5s/VERSION_NUMBER/$(VERSION)/' > install.rdf
	zip -r Mpris-$(VERSION)-32bit.xpi components/ chrome/ platform/ install.rdf chrome.manifest
	
package64: build
	sed '7s/MPRIS/MPRIS (64-bit)/' src/install.rdf | sed '5s/UUID/$(UUID64)/' | sed '5s/VERSION_NUMBER/$(VERSION)/' > install.rdf
	zip -r Mpris-$(VERSION)-64bit.xpi components/ chrome/ platform/ install.rdf chrome.manifest
